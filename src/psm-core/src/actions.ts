...
export function normalizeAction(action: unknown): MemoryAction {
...
export function routeForAction(action: MemoryAction): MemoryRoute {
  switch (action) {
    case "ignore":
      return "ignore";
    case "rank":
...
export function actionFromOperation(operation: string): MemoryAction {
  const value = operation.toLowerCase();
  if (value.includes("ignore")) return "ignore";
  if (value.includes("interference") || value.includes("conflict")) return "detect_interference";
  if (value.includes("promote")) return "promote";
  if (value.includes("update")) return "update_existing";
  if (value.includes("rank") || value.includes("recall")) return "rank";
  if (value.includes("decay")) return "decay";
  return "store_episodic";
}
