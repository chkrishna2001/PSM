...
`);
    ...
    applyDecision(userId, source, decision, extraTags = []) {
    ...
    insertEpisodic(userId, content, memory = {}) {
        const id = randomUUID();
        this.db.prepare(`
INSERT INTO episodic (id, user_id, content, strength, decay_rate, emotional_weight, confidence, tags, promoted)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0)`).run(id, userId, content, memory.strength ?? 0.75, memory.decay_rate ?? 0.02, memory.emotional_weight ?? 0.2, memory.confidence ?? 0.8, JSON.stringify(memory.tags ?? []));
        return id;
    }
    insertSemantic(userId, content, memory = {}, sourceEpisodes = []) {
        const id = randomUUID();
        this.db.prepare(`
INSERT INTO semantic (id, user_id, content, strength, decay_rate, emotional_weight, confidence, tags, source_episodes)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, userId, content, memory.strength ?? 0.85, memory.decay_rate ?? 0.005, memory.emotional_weight ?? 0.2, memory.confidence ?? 0.85, JSON.stringify(memory.tags ?? []), JSON.stringify(memory.source_episodes ?? sourceEpisodes));
        return id;
    }
    insertConflict(userId, content, reason) {
        const id = randomUUID();
        this.db.prepare(`
...
VALUES (?, ?, ?, ?, 'unresolved')`).run(id, userId, content, reason);
    ...
    insertDecaySchedule(userId, memoryKey, decayRate) {
        const id = randomUUID();
        this.db.prepare(`
...
VALUES (?, ?, ?, datetime('now', '+1 day'), ?)`).run(id, userId, memoryKey, decayRate);
    ...
    insertDecision(userId, source, action, route, reasoning, rawJson) {
        const id = randomUUID();
        this.db.prepare(`
...
VALUES (?, ?, ?, ?, ?, ?, ?)`).run(id, userId, source, action, route, reasoning, rawJson);
    ...
    upsertMemoryEmbedding(ref, userId, model, embedding) {
        this.db.prepare(`
...
ON CONFLICT(memory_table, memory_id, model) DO UPDATE SET
  ...
  updated_at = CURRENT_TIMESTAMP`).run(ref.table, ref.id, userId, model, embedding.length, JSON.stringify(embedding), hashContent(ref.content));
    }
    selectEmbeddingRows(userId, model) {
...
function withExtraTags(memory, extraTags) {
