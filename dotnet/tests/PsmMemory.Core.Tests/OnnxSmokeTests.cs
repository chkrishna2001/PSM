...
/// <summary>
/// Integration smoke test: loads the real OnnxPsmRuntime against the validated model at
/// psm-model/prod-memory/onnx-runtime/v1 and drives PsmService.RememberAsync end-to-end (real
...
/// 100-case gate. Skipped by default because it loads a real ~1GB+ model and does real greedy
/// decoding (tens of seconds); this was run manually once during development (see the port's
/// completion report for the captured JSON output) to confirm the plumbing is sound.
/// </summary>
public class OnnxSmokeTests
{
    ...
    [Fact(Skip = "manual — requires the real ONNX model on disk; run manually to validate the ONNX plumbing end-to-end")]
    public async Task RememberAsync_ProducesWellFormedDecisions_ForRealGateCases()
    {
        var modelDir = Path.Combine(FindRepoRoot(), "psm-model", "prod-memory", "onnx-runtime", "v1");
        Assert.True(Directory.Exists(modelDir), $"expected model directory at {modelDir}");

        using var runtime = new OnnxPsmRuntime(modelDir);
