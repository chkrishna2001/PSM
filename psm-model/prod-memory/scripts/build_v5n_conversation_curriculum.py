#!/usr/bin/env python3
"""Build hf-prod-v5n.jsonl — v5h JSON mix + Gemma conversation teacher anchors (no LoCoMo)."""
...
DEFAULT_CONVERSATION = PACKAGE_ROOT / "data" / "hf-prod-conversation-gemma-combined.jsonl"
DEFAULT_OUT = PACKAGE_ROOT / "data" / "hf-prod-v5n.jsonl"
...
def main() -> int:
    ...
    parser.add_argument("--store-copies", type=int, default=2, help="Extra copies of non-ignore conversation rows.")
    ...
    store_rows = [
        row for row in conversation if str(row.get("expected", {}).get("action") or "") not in {"ignore", "ignore_noise"}
    ]
    if args.store_copies > 1 and store_rows:
        anchors.extend(_copy_rows(store_rows, prefix="conv", copies=args.store_copies - 1))

    seed = build_json_fixture_rows()
    anchors.extend(seed)
    anchors.extend(_copy_rows(seed, prefix="fxn", copies=7))
    boost_seed = [row for row in seed if any(fid in row["id"] for fid in V5D_BOOST_FIXTURE_IDS)]
    if boost_seed:
        anchors.extend(_copy_rows(boost_seed, prefix="fxnb", copies=30))
