from __future__ import annotations

import argparse
import hashlib
import json
from pathlib import Path
from typing import Any

from psm_model.data import validate_training_row


PREFERENCES = [
    ("concise technical answers", "answer_style", "concise_technical_answers"),
    ("PowerShell one-line commands", "command_style", "powershell_one_line"),
    ("SQLite for local prototypes", "tool_preference", "sqlite_local_prototypes"),
    ("checkpoint gates before long training", "training_process", "checkpoint_gates_first"),
    ("small focused diffs", "coding_style", "small_focused_diffs"),
    ("direct evidence in memory facts", "memory_quality", "direct_evidence"),
]

EVENTS = [
    ("ran the action-prefix gate", "action_prefix_gate", "completed"),
    ("uploaded the dataset snapshot", "dataset_snapshot", "uploaded"),
    ("fixed the malformed fact parser", "fact_parser", "fixed"),
    ("reduced CUDA memory fraction to 0.65", "cuda_memory_fraction", "0.65"),
    ("created the mixed repair checkpoint", "mixed_repair_checkpoint", "created"),
    ("validated the concept probe", "concept_probe", "validated"),
]

RULES = [
    ("ask before deleting checkpoints", "checkpoint_deletion", "requires_confirmation"),
    ("gate datasets before training", "dataset_training", "gate_first"),
    ("run direct probes after every checkpoint", "checkpoint_evaluation", "direct_probe_required"),
    ("avoid using CPU torch for GPU training", "training_runtime", "cuda_torch_required"),
    ("keep generated checkpoints out of git", "source_control", "exclude_checkpoints"),
    ("use action diagnostics before full validation", "validation_order", "action_diagnostics_first"),
]

NOISE = [
    "okay thanks",
    "that worked",
    "please continue",
    "I will paste the JSON",
    "the terminal is still running",
    "sounds good",
]


def build_rows(rows_per_action: int) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for index in range(rows_per_action):
        pref, predicate, value = PREFERENCES[index % len(PREFERENCES)]
        rule, rule_predicate, rule_value = RULES[index % len(RULES)]
        event, event_subject, event_value = EVENTS[index % len(EVENTS)]
        noise = NOISE[index % len(NOISE)]
        date = f"2026-06-{(index % 20) + 1:02d}"

        rows.append(
            row(
                f"foundation-promote-{index}",
                base_input(f"User: I prefer {pref}.", index),
                expected_memory(
                    "promote_semantic",
                    "semantic",
                    f"The user prefers {pref}.",
                    ["preference", predicate],
                    "User",
                    "prefers",
                    value,
                    f"I prefer {pref}.",
                    "The user stated an explicit durable preference.",
                ),
            )
        )
        rows.append(
            row(
                f"foundation-store-{index}",
                base_input(f"User: On {date}, I {event}.", index, source_timestamp=f"{date}T12:00:00Z"),
                expected_memory(
                    "store_episodic",
                    "episodic",
                    f"On {date}, the user {event}.",
                    ["event", event_subject],
                    "User",
                    event_subject,
                    event_value,
                    f"On {date}, I {event}.",
                    "The user described a completed dated event.",
                    temporal_expression=date,
                    resolved_time=date,
                    decay_rate=0.05,
                ),
            )
        )
        rows.append(
            row(
                f"foundation-update-{index}",
                base_input(
                    f"User: Correction: use {pref} instead.",
                    index,
                    context=json.dumps({"memory_store": [{"id": "m1", "content": f"User previously preferred {rule}."}]}),
                ),
                expected_memory(
                    "update_existing",
                    "semantic",
                    f"Update the stored preference: the user now prefers {pref}.",
                    ["update", predicate],
                    "User",
                    "updated_preference",
                    value,
                    f"Correction: use {pref} instead.",
                    "The current message replaces prior stored memory.",
                ),
            )
        )
        rows.append(
            row(
                f"foundation-conflict-{index}",
                base_input(
                    f"User: Actually, do not {rule}; keep the previous behavior.",
                    index,
                    context=json.dumps({"memory_store": [{"id": "m1", "content": f"For this repo, {rule}."}]}),
                ),
                expected_memory(
                    "flag_conflict",
                    "semantic",
                    f"The user contradicted the prior rule: {rule}.",
                    ["conflict", rule_predicate],
                    "this repo",
                    "conflicts_on",
                    rule_value,
                    f"Actually, do not {rule}; keep the previous behavior.",
                    "The message conflicts with an existing stored rule.",
                ),
            )
        )
        rows.append(
            row(
                f"foundation-flag-store-{index}",
                base_input(f"User: Correction: do not {rule}; instead, {PREFERENCES[(index + 1) % len(PREFERENCES)][0]}.", index),
                expected_memory(
                    "flag_and_store",
                    "semantic",
                    f"Correction: do not {rule}; instead use {PREFERENCES[(index + 1) % len(PREFERENCES)][0]}.",
                    ["correction", rule_predicate],
                    "this repo",
                    "corrected_rule",
                    rule_value,
                    f"Correction: do not {rule}; instead, {PREFERENCES[(index + 1) % len(PREFERENCES)][0]}.",
                    "The user provided a correction that should be stored.",
                ),
            )
        )
        rows.append(row(f"foundation-ignore-{index}", base_input(f"User: {noise}.", index), expected_ignore("No durable memory value.")))
    return rows


def base_input(conversation: str, index: int, *, context: str | None = None, source_timestamp: str | None = None) -> dict[str, Any]:
    payload: dict[str, Any] = {
        "conversation": conversation,
        "operation": "remember",
        "source_id": f"foundation-{index}",
        "source_kind": "action_foundation",
        "source_timestamp": source_timestamp or "2026-06-03T12:00:00Z",
    }
    if context is not None:
        payload["context"] = context
    return payload


def expected_ignore(reasoning: str) -> dict[str, Any]:
    return {"action": "ignore", "memory": None, "facts": [], "reasoning": reasoning}


def expected_memory(
    action: str,
    memory_type: str,
    content: str,
    tags: list[str],
    subject: str,
    predicate: str,
    value: str,
    evidence: str,
    reasoning: str,
    *,
    temporal_expression: str | None = None,
    resolved_time: str | None = None,
    decay_rate: float = 0.02,
) -> dict[str, Any]:
    memory: dict[str, Any] = {
        "type": memory_type,
        "content": content,
        "strength": 0.86,
        "decay_rate": decay_rate,
        "emotional_weight": 0.2,
        "confidence": 0.92,
        "tags": tags,
    }
    if temporal_expression:
        memory["temporal_expression"] = temporal_expression
    if resolved_time:
        memory["resolved_time"] = resolved_time
    fact: dict[str, Any] = {
        "subject": subject,
        "predicate": predicate,
        "value": value,
        "confidence": 0.92,
        "inference_kind": "explicit",
        "evidence_text": evidence,
    }
    if temporal_expression:
        fact["temporal_expression"] = temporal_expression
    if resolved_time:
        fact["resolved_time"] = resolved_time
    return {"action": action, "memory": memory, "facts": [fact], "reasoning": reasoning}


def row(row_id: str, input_payload: dict[str, Any], expected: dict[str, Any]) -> dict[str, Any]:
    result = {"id": row_id, "input": input_payload, "expected": expected, "source": "action_foundation_curriculum"}
    _, issues = validate_training_row(result)
    if issues:
        formatted = ", ".join(f"{issue.path}: {issue.message}" for issue in issues)
        raise ValueError(f"{row_id}: {formatted}")
    return result


def split_rows(rows: list[dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
    splits = {"train": [], "validation": [], "test": []}
    for item in rows:
        bucket = int(hashlib.sha256(item["id"].encode("utf-8")).hexdigest()[:8], 16) / 0xFFFFFFFF
        if bucket < 0.1:
            splits["test"].append(item)
        elif bucket < 0.25:
            splits["validation"].append(item)
        else:
            splits["train"].append(item)
    return splits


def write_jsonl(path: Path, rows: list[dict[str, Any]]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(json.dumps(row, ensure_ascii=False, sort_keys=True) for row in rows) + "\n", encoding="utf-8")


def main() -> int:
    parser = argparse.ArgumentParser(description="Generate deterministic all-action PSM foundation curriculum rows.")
    parser.add_argument("output_dir", type=Path)
    parser.add_argument("--rows-per-action", type=int, default=600)
    args = parser.parse_args()

    rows = build_rows(args.rows_per_action)
    splits = split_rows(rows)
    for split, split_rows_value in splits.items():
        write_jsonl(args.output_dir / f"{split}.jsonl", split_rows_value)
    write_jsonl(args.output_dir / "all.jsonl", rows)
    report = {
        "output_dir": str(args.output_dir),
        "rows": len(rows),
        "rows_per_action": args.rows_per_action,
        "splits": {split: len(split_rows_value) for split, split_rows_value in splits.items()},
    }
    (args.output_dir / "metadata.json").write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    print(json.dumps(report, indent=2, sort_keys=True))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
