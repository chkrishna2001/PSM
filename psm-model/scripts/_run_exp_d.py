#!/usr/bin/env python3
"""Deploy pod and run Experiment D (two-pass inference, separate checkpoints)."""
...
def _deploy() -> tuple[str, str]:
    proc = subprocess.run(
        [sys.executable, str(SCRIPTS / "runpod_ctl.py"), "deploy", "--auto-gpu", "--name", "psm-exp-d", "--wait-ssh", "300"],
        cwd=REPO,
        capture_output=True,
        text=True,
    )
...
def main() -> int:
    ...
    extra = {
        ...
        "EXP_D_CONTEXT_LENGTH": "2048",
    }
    rc_ = rc._ssh_run_script(
        "runpod-psm-proxy",
        SCRIPTS / "runpod_exp_d_two_pass.sh",
        host=ssh_host,
        port=ssh_port,
        user=ssh_user,
        timeout_sec=9000,
