#!/bin/bash
# Experiment A: minimal one-line format capacity test (3 fixtures, 800 steps).
set -euo pipefail
...
RESUME_STEP="${EXP_A_RESUME_STEP:-058000}"
STEM="${EXP_A_STEM:-real-v3-50m-full-v2}"
OUT_STEM="${EXP_A_OUT_STEM:-real-v3-50m-exp-a-minimal}"
FIXTURE_IDS="${EXP_A_FIXTURE_IDS:-cursor-01-summary,cursor-02-debug,plan-01-handoff}"
TARGET_STEPS="${EXP_A_TARGET_STEPS:-800}"
LR="${EXP_A_LR:-3e-5}"
CTX="${EXP_A_CONTEXT_LENGTH:-2048}"
ACTION_W="${EXP_A_ACTION_LOSS_WEIGHT:-1.0}"

echo "=== exp-a minimal overfit $(date -u +%Y-%m-%dT%H:%M:%SZ) resume=$RESUME_STEP ctx=$CTX fixtures=$FIXTURE_IDS ==="
...
out = Path("psm-model/prod-memory/data/exp-a-minimal-fixtures.jsonl")
n = write_minimal_fixture_jsonl(out, ids)
...
python3 -m psm_model.train \
  psm-model/prod-memory/data/exp-a-minimal-fixtures.jsonl \
...
EVAL_OUT="psm-model/prod-memory/results/exp-a-minimal-${FINAL_STEP}.json"
python3 -m prod_memory.eval_grounding \
  --checkpoint "$CKPT" \
  --checkpoint-label "exp-a-${FINAL_STEP}" \
...
passed = sum(1 for c in r.get("cases", []) if c.get("id") in want and c.get("effective_stored"))
print(json.dumps({"trained_fixtures_pass": f"{passed}/{len(want)}", "pass_criterion": "3/3 effective_stored"}))
for c in r.get("cases", []):
    if c.get("id") in want:
        print(json.dumps({"fixture": c["id"], "effective_stored": c["effective_stored"],
                          "memory_content": c.get("memory_content"), "action": c.get("action")}))
PY
...
for path in [
    Path("$EVAL_OUT"),
    Path("psm-model/prod-memory/data/exp-a-minimal-fixtures.jsonl"),
]:
    if path.exists():
        api.upload_file(path_or_fileobj=str(path), path_in_repo=path.as_posix(), repo_id=repo, repo_type="model",
                        commit_message=f"exp-a minimal {path.name}")
        print("uploaded", path.name)
PY
fi

echo "=== exp-a minimal done ==="
