#!/usr/bin/env python3
"""Deploy pod and run Experiment B (binary classify overfit)."""
...
def _resolve_pod(deploy: bool) -> tuple[str, str]:
    pod_id = ""
    proxy_user = ""
    if deploy:
        proc = subprocess.run(
            [sys.executable, str(SCRIPTS / "runpod_ctl.py"), "deploy", "--auto-gpu", "--name", "psm-exp-b", "--wait-ssh", "300"],
            cwd=REPO,
            capture_output=True,
            text=True,
        )
        ...
        for line in proc.stdout.splitlines():
            ...
            if payload.get("pod_host_id"):
                proxy_user = payload["pod_host_id"]
...
def main() -> int:
    ...
    if args.deploy and not pod_id:
        pod_id, proxy_user = _resolve_pod(True)
    ...
    extra = {
        ...
        "EXP_B_CONTEXT_LENGTH": "2048",
    }
    rc_ = rc._ssh_run_script(
        "runpod-psm-proxy",
        SCRIPTS / "runpod_exp_b_classify_overfit.sh",
        host=ssh_host,
        port=ssh_port,
        user=ssh_user,
        timeout_sec=5400,
